...
async def setup_mail_gw(**kwargs) -> Tuple[str, str, Dict[str, Any]]:
    sess = make_requests_session()
    return await _setup_internal_mail_tm_gw_like(MAILGW_BASE_URL, "mail.gw", sess)
...
async def fetch_mail_gw_messages(
    ...
    return await _fetch_internal_mail_tm_gw_like_messages(
        provider_data, "mail.gw", sess, active_sessions_ref, save_sessions_func
    )
